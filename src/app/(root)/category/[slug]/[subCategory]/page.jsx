...
import { buildMetadata, extractCategoryIds, findCategoryData, getAllCategoriesPathsMultiple } from "@/Utils/Utils";
import { notFound } from "next/navigation";

export async function generateMetadata({ params }) {
  try {
    const decodedSlug = decodeURIComponent(params.subCategory);
    ...
    let selectedCategoryData = findCategoryData(categoriesData, slug) || findCategoryData(categoriesData, "/category/" + params.subCategory);
...
export const generateStaticParams = async () => {
  try {
    ...
    const formattedPaths = paths.map(({ slug, subCategory }) => ({ slug, subCategory }));
...
export default async function Page({ params }) {
  try {
    const slug = "/category/" + decodeURIComponent(params.subCategory);

    const categoriesData = await fetchAllCategoriesData();

    const selectedCategoryData = findCategoryData(categoriesData, slug) || findCategoryData(categoriesData, "/category/" + params.subCategory);
